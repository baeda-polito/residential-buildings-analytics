...
def replace_constant_values(series: pd.Series, n: int) -> pd.Series:
    """
    Individua i valori costanti in una serie temporale e li sostituisce con NaN.
    Args:
        series (pd.Series): la serie temporale
        n (int): numero massimo di valori costanti consecutivi ammesso
    Returns:
        pd.Series: la serie storia ripulita dai valori costanti
    """
...
def reconstruct_missing_values_interp(data: pd.DataFrame, max_missing: int = 4) -> pd.DataFrame:
    """
    Ricostruisce i valori mancanti in un profilo di carico giornaliero interpolando i valori mancanti consecutivi fino a
    max_missing.
    Args:
        data (pd.DataFrame): dataframe con i valori mancanti. Deve contenere la colonna power e timestamp
        max_missing (int): numero
    Returns:
        pd.DataFrame: dataframe ricostruito con i valori mancanti imputati
...
def reconstruct_missing_values_knn(df: pd.DataFrame, k: int = 5, min_missing: int = 5, max_missing: int = 16):
    """
    ...
    Args:
        df (pd.DataFrame): dataframe con i valori mancanti. Deve contenere la colonna power e timestamp
        k (int): numero di vicini da considerare in KNN
        min_missing (int): numero
        max_missing (int): numero
    Returns:
        pd.DataFrame: dataframe ricostruito con i valori mancanti imputati
    """
    ...
    df_reconstructed = daily_profiles.stack(future_stack=True).reset_index()
