...
from . import shape_factors


def calculate_shape_factor(data: pd.DataFrame) -> pd.DataFrame:
    """
    Calcola i fattori di forma giornalieri per una serie temporale di potenza.

    Args:
        data (pd.DataFrame): Dataframe con la colonna "Load", contenente i dati di potenza elettrica consumata, e timestamp index.

    Returns:
        pd.DataFrame: Dataframe con i fattori di forma calcolati per ogni giorno disponibile.
    """

    mean_max = shape_factors.sf_mean_max(data)
    min_max = shape_factors.sf_min_max(data)
    min_mean = shape_factors.sf_min_mean(data)
    daytime_mean = shape_factors.sf_daytime_mean(data)
    daytime_max = shape_factors.sf_daytime_max(data)
    daytime_min_mean = shape_factors.sf_daytime_min_mean(data)
    nightime_mean = shape_factors.sf_nightime_mean(data)
    nightime_max = shape_factors.sf_nightime_max(data)
    nightime_min_mean = shape_factors.sf_nightime_min_mean(data)
    afternoon_mean = shape_factors.sf_afternoon_mean(data)
    afternoon_max = shape_factors.sf_afternoon_max(data)
    afternoon_min_mean = shape_factors.sf_afternoon_min_mean(data)
    peak_load = shape_factors.peakload(data)
    peak_period = shape_factors.peak_period(data, peak_load)
...
def find_medioid_and_quartiles(df_cluster, centroids_pivot):
    ...
    Args:
        df_cluster (pd.DataFrame): Dataframe con i profili dei consumatori e prosumer.
        centroids_pivot (pd.DataFrame): Dataframe con i centroidi dei cluster in pivot.

    Returns:
        Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]: Medioidi, Q1 e Q3 come DataFrames.
    """
    nearest_profiles = {}
    q1_profiles = {}
    q3_profiles = {}
